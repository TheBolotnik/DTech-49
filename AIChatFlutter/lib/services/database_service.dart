...
class DatabaseService {
  ...
  Future<Database> _initDatabase() async {
    ...
    return await openDatabase(
      ...
      onCreate: (Database db, int version) async {
        ...
        ''');
        // Создание таблицы для хранения учетных данных
        await db.execute('''
      ...
      onUpgrade: (Database db, int oldVersion, int newVersion) async {
        if (oldVersion < 2) {
          // Миграция: добавление таблицы app_credentials
          await db.execute('''
            CREATE TABLE app_credentials (
              key TEXT PRIMARY KEY,
              value TEXT NOT NULL
  ...
  Future<void> saveCredentials(Map<String, dynamic> json) async {
    try {
      final db = await database;
      final jsonString = const JsonEncoder().convert(json);
      await db.insert(
        'app_credentials',
        {'key': 'app_credentials', 'value': jsonString},
