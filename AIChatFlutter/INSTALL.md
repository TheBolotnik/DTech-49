...
2. Настройте параметры приложения в .env (при необходимости):
```
...
**ВАЖНО:** API ключи НЕ хранятся в .env файле. При первом запуске приложение запросит:
- Ввод API ключа (OpenRouter или VseGPT)
- Создание PIN-кода для защиты

Все учетные данные шифруются и хранятся в защищенном локальном хранилище.
